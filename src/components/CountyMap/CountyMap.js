...
/*
	Need three D3 objects to build a choropleth map:
		- geographical projection
		- path generator
		- quantize scale for colors
*/
class CountyMap extends Component {
  ...
  updateD3(props) {
    ...
    if (props.zoom && props.usTopoJson) {
      // get list of us states, find one to zoom in on
      ...
      // make map bigger, creates zooming effect
    ...
    // update the quantize scales domain
    // creates more visual contrast
    if (props.values) {
  ...
  render() {
    if (!this.props.usTopoJson) {
      ...
      const us = this.props.usTopoJson,
        // removes duplicate boundaries (borders between states)
